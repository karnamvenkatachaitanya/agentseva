...
def test_root(client: TestClient):
    ...
    if frontend_dist.is_dir():
        ...
        assert "AgentSeva" in resp.text
    else:
        # Static assets are generated by the production build and are
...
def test_health(client: TestClient):
    ...
    assert "database" in deps and "razorpay_mode" in deps and "huggingface_configured" in deps
...
def test_chat_without_api_key_returns_503(client: TestClient):
    # In the test environment HUGGINGFACE_API_KEY is unset → structured 503.
    from app.core.config import settings

    if settings.HUGGINGFACE_API_KEY:
        pytest.skip("HUGGINGFACE_API_KEY is configured; live agent path not exercised in tests.")
